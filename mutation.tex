...
Applying mutation testing extensively to real-world code, in a situation where we aim to collect feedback from developers (e.g. at NASA/JPL) will also allow us to further investigate the relationship between code coverage and mutation testing, and in particular the enduring problem that fuzzing can have very high code coverage and poor mutation awareness~\cite{MindGap}, a major thrust of our most recently NSF-funded work.  Applying mutation analysis, with a large and diverse mutant population, to FPrime projects that, almost always, should have high-coverage manually constructed test suites, will enable us to evaluate and refine our approach to adding more specification-visibility via runtime assertions that can be checked during normal tests and as part of fuzzing or model checking efforts.  When a mutant is detected by manual tests but not by fuzzing we can attempt to determine whether this is an inherent limitation of test-generation approaches where predicting proper behavior is harder than with crafted scenarios, or if the underlying problem is a missing runtime assertion.  We speculate that some framework-aligned custom mutations may inspire new types of runtime checks relating to framework semantic consistency, and these can thus also be evaluated with such mutations.